...
import { TEACHER_CATEGORIES_PER_PAGE } from "../constants";
import { useRouter, useRoute } from "vue-router";
const router = useRouter();
const perPage = ref(TEACHER_CATEGORIES_PER_PAGE);
const search = ref("");
const sortField = ref("updated_at");
const sortDirection = ref("desc");

const categoires = computed(() => store.state.teacherCategories);
...
const deleteCategory = (category) => {
  ...
  store.dispatch("deleteTeacherCategory", category.id).then((res) => {
    alert("刪除成功！");
    getCategories();
  });
};
...
const deleteCheckedItems = () => {
  if (confirm("確定刪除？")) {
    store.dispatch("deleteTeacherCategoryItems", checkedItems.value).then((res) => {
      alert("刪除成功！");
      getCategories();
      checkedItems.value = [];
    });
  ...
  <div class="categoires">
    ...
    <div class="card">
      <div class="card-header">
        ...
        <div class="right">
          <div class="form-group">
            <router-link
              class="btn"
              :to="{ name: 'app.teacher.add-category', params: { id: 'create' } }"
              >+ 新增師資科目</router-link
            >
          </div>
        </div>
      ...
      <div class="table-responsive">
        <table class="table table-auto w-full animate-fade-in-down">
          ...
          <tbody v-else>
            <tr>
              <td colspan="7" class="w-full" style="text-align: center">
                <div class="py-3 text-center text-xs text-gray-500" v-if="categoires.total <= 0">暫無資料</div>
              </td>
            </tr>
            <tr
      ...
      <div class="paging" v-if="categoires.total > categoires.limit">
        <div class="pageInfo text-xs text-gray-500">第 {{ categoires.from }} 筆 至 第 {{ categoires.to }}筆</div>
        <div class="pageBtn">
          <nav>
            <a
              href="#"
