...
onMounted(() => {
  ...
  store
    .dispatch("isExistStudent", studentId)
    .then((res) => {
      if (res.data) {
        store
          ...
          .then((res) => {
            ...
            student.value.name =
              student.value.name == "null" ? "" : student.value.name;
            student.value.content =
              student.value.content == "null" ? "" : student.value.content;
          })
          .then(() => {
            if (image_url.value != "") {
...
const previewImage = (ev) => {
  ...
  if (ev.target.files && ev.target.files[0]) {
    student.value.image = ev.target.files[0];
...
const onSubmit = () => {
  ...
  if (isCreate.value) {
    store
      .dispatch("createStudent", student.value)
      .then((res) => {
    ...
    store
      .dispatch("updateStudent", student.value)
      .then((res) => {
        if (res.status === 200 || res.status === 201) {
    ...
    <div class="card">
      ...
      <form v-if="randerLoading" action="" @submit.prevent="onSubmit()">
        <div class="form-group">
          <label for=""><span class="text-red-800">*</span>學生姓名</label>
          <input type="text" v-model="student.name" @keyup="successMsgSetNull()" />
        </div>
        <div class="form-group">
          <label for=""><span class="text-red-800">*</span>感言內容</label>
          <ckeditor
            :editor="editor"
            id="editor"
          ...
          ></ckeditor>
          <!-- <textarea id="editor" name="editor" v-model="student.content"></textarea> -->
        </div>
        <div class="form-group">
          <label for=""><span class="text-red-800"></span>Youtube連結</label>
          <input type="text" v-model="student.url" @keyup="successMsgSetNull()" />
        </div>
        <div class="form-group">
          <label for="">學生圖片</label>
        ...
        <div class="chkbox-group">
          <div class="form-group">
            <label for="">隱藏學生</label>
            <input type="checkbox" v-model="student.hidden" @keyup="successMsgSetNull()" />
          </div>
        </div>
        ...
        <div class="form-group btn-group mt-10">
          ...
          <button
            class="pre"
            type="button"
            @click="router.push({ name: 'app.students' })"
          >
            回列表
          </button>
        </div>
