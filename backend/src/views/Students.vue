...
const deleteStudent = (student) => {
  ...
  store.dispatch("deleteStudent", student.id).then((res) => {
    alert("刪除成功！");
    getStudents();
  });
};
const selectedCheckItem = () => {
  if (checkedItems.value.length < students.value.total) {
...
const deleteCheckedItems = () => {
  if (confirm("確定刪除？")) {
    store.dispatch("deleteStudentItems", checkedItems.value).then((res) => {
      alert("刪除成功！");
      getStudents();
      checkedItems.value = [];
    });
    ...
    <div class="card">
      <div class="card-header">
        <div class="left">
          ...
          <div class="form-group">
            <select v-model="perPage" @change="getStudents()">
        ...
        <div class="right">
          <div class="form-group">
            <router-link
              class="btn"
              :to="{ name: 'app.add-student', params: { id: 'create' } }"
              >+ 新增回饋</router-link
            >
          </div>
        </div>
      ...
      <div class="table-responsive">
        <table class="table table-auto w-full animate-fade-in-down">
          ...
          <tbody v-if="students.loading" class="loadingTable">
            <tr>
          ...
          <tbody v-else>
            <tr>
              <td colspan="7" class="w-full" style="text-align: center">
                <div class="py-3 text-center text-xs text-gray-500" v-if="students.total <= 0">暫無資料</div>
              </td>
            </tr>
            <tr
              v-for="(student, idx) of students.data"
              :key="student.id"
            ...
            >
              ...
              <td class="w-[40px]">{{ student.id }}</td>
              <td>
                <img v-if="student.image_url" :src="student.image_url" />
                <img v-else src="@/assets/news.jpg" />
              </td>
              <td>{{ student.name }}</td>
              <td>{{ student.updated_at }}</td>
              <td>
                <span v-if="student.hidden">隱藏</span
              ...
              <td>
                <button
                  class="edit ml-1"
                  @click="
                    router.push({ name: 'app.add-student', params: { id: student.id } })
                  "
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                ...
                <button class="delete ml-5" @click="deleteStudent(student)">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
      ...
      <div class="paging" v-if="students.total > students.limit">
        <div class="pageInfo text-xs text-gray-500">第 {{ students.from }} 筆 至 第 {{ students.to }}筆</div>
        <div class="pageBtn">
          <nav>
            <a
              href="#"
